...
#     def __init__(self):
#         self.time_dict = {}
#         self.time_steps = 60 * 2  # idő váltások másodpercben
#         self.get_time_dict()
...
def save_data(name, df):
    ...
    print(name + ".pickle saved")



symbols = ['bchusdt', 'dotusdt', 'algousdt', 'shibusdt', 'manausdt',
           'sandusdt', 'etcusdt', 'linkusdt', 'lunausdt', 'nearusdt', 'ethusdt',
           'btcusdt', 'trxusdt', 'lrcusdt', 'omgusdt', 'ftmusdt', 'solusdt',
           'vetusdt', 'roseusdt', 'avaxusdt', 'atomusdt', 'thetausdt',
           'oneusdt', 'xrpusdt', 'nmrusdt']

column_names = ['lastUpdateId',
                'bids',
                'asks',
                'datetime']
ndf_book = {}
n_array = {}
for n in symbols:
    ndf_book[n] = pd.DataFrame(columns=column_names)
    n_array[n] = []
...
for n_prefix in range(500):
    ...
    if created_name in file_names:
        print(created_name)
        print("kezd")
        res = pickle.load(open(data_path + "/" + created_name, "rb"))
        print("végez")
        for res_line in res:
            # print(res[res_line])
            pd_dic = {}
            # pd_dic['stream'] = res[res_line]['stream']
            pd_dic['lastUpdateId'] = res[res_line]['data']['lastUpdateId']
            pd_dic['bids'] = res[res_line]['data']['bids']
            pd_dic['asks'] = res[res_line]['data']['asks']
            pd_dic['datetime'] = res[res_line]['datetime']
            i_symbol = res[res_line]['stream'].replace('@depth20', '')
            n_array[i_symbol].append(pd_dic)

for n in symbols:
    print(n, "1", len(n_array[n]))
    ndf_book[n] = pd.DataFrame(n_array[n])
    print(n, "2")
    # print("df_book_in", n, len(df_book_in))
    # ndf_book[n] = pd.concat([ndf_book[n], df_book_in])
    # print("ndf_book", n, len(ndf_book[n]))
    ...
    # gc.collect()
            ...
            # df_book.loc[len(df_book)] = pd_dic
    print("--------------")
# df_book['ids'] = df_book['stream'] + str(df_book['lastUpdateId'])
# print(df_book[df_book.duplicated(['ids'], keep=False)])
# for n in symbols:
    print("save:", n)
...
# df.reset_index(drop=True, inplace=True)
    ndf_book[n]['lastUpdateId'] = ndf_book[n]['lastUpdateId'].astype('int64')

    # ndf_book[n].set_index("lastUpdateId", inplace=True)

    # print(len(ndf_book[n]))
    ndf_book[n].drop_duplicates('lastUpdateId', keep='last', inplace=True)
    ndf_book[n].sort_values(by=['datetime'], inplace=True, ascending=True)
    ndf_book[n].reset_index(drop=True, inplace=True)
    # print(len(ndf_book[n]))
    # print(n, "."*20)
    # print(len(ndf_book[n]))
    save_data(n, ndf_book[n].copy())
    print(ndf_book[n])

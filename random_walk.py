...
class n_arbitrage:
    ...
    def arb_matrix(self):
        ...
        while True:
            ...
            if del_row_id == -1:
                ...
                self.maxi_pairs = np.delete(self.maxi_pairs, del_row_id, 0)
    ...
    async def asyc_websocket(self):
        global ab1, ab2, ab3, last_arb, trade_in_progress
        ...
        async with ts as tscm:
            while True:
                ...
                np.put(ab3, self.refresh_map[s1s2][2], bid)
    ...
    def random_walk(self):
        ...
        max_row = random.randint(0, len(ab1))
        profit = ab1[max_row] * ab2[max_row] * ab3[max_row]
        ...
        pairs = {}
        for sy1 in self.symbols:
            for sy2 in self.symbols:
                pairs[sy1+sy2] = [sy1, sy2]
                pairs[sy2+sy1] = [sy2, sy1]

        symbol_f1 = self.maxi_pairs[max_row][0].decode('UTF-8')
        symbol_f2 = self.maxi_pairs[max_row][1].decode('UTF-8')
        symbol_f3 = self.maxi_pairs[max_row][2].decode('UTF-8')

        symbol_b1 = pairs[symbol_f3][1] + pairs[symbol_f3][0]
        symbol_b2 = pairs[symbol_f2][1] + pairs[symbol_f2][0]
        symbol_b3 = pairs[symbol_f1][1] + pairs[symbol_f1][0]

        print(symbol_b1, symbol_b2, symbol_b3)
